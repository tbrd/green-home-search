...
from main import get_running_cost  # noqa: E402
...
def test_search_returns_results(client):
    ...
    if data["total"] > 0:
        ...
        assert first_result["running_cost"] is None or isinstance(
            first_result["running_cost"], (int, float)
        )
...
def test_search_with_filters(client):
    ...
    response = client.get(
        "/search?address=London&energy_rating=C&property_type=House&limit=3"
    )
...
def test_search_with_efficiency_range(client):
    ...
    response = client.get(
        "/search?address=Manchester&min_efficiency=60&max_efficiency=80&limit=5"
    )
...
def test_search_includes_running_cost(client):
    ...
    if data["total"] > 0 and len(data["results"]) > 0:
        for result in data["results"]:
            ...
            if (
                result.get("heating_cost_current") is not None
                and result.get("hot_water_cost_current") is not None
            ):
                ...
                expected_cost = (
                    result["heating_cost_current"] + result["hot_water_cost_current"]
                )
